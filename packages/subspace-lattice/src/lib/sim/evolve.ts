...
export function formatEvolutionReport(result: EvolutionRunResult): string {
  ...
  for (const c of result.scorecards) {
    ...
    lines.push(
      `  ${c.rejected ? 'REJECT' : 'OK   '} ${c.configId} W=${whitePercent.toFixed(0)}% B=${blackPercent.toFixed(0)}% paths W[h${whitePaths.hubCapture}/s${whitePaths.sectorIntegration}/n${whitePaths.noMoves}] B[h${blackPaths.hubCapture}/s${blackPaths.sectorIntegration}/n${blackPaths.noMoves}] composite=${c.composite.toFixed(3)} fair=${c.fairness.toFixed(2)} cal=${c.skillCalibration.toFixed(2)} sep=${c.skillSeparation.toFixed(1)} σ=${c.skillMeanSigma.toFixed(1)} mid=${c.interestingMidgame.toFixed(2)} avgPlies=${c.avgPlies.toFixed(1)} hub=${(wp.hubCaptureRate * 100).toFixed(0)}% sec=${(wp.sectorIntegrationRate * 100).toFixed(0)}% lock=${(wp.noMovesRate * 100).toFixed(0)}% emp=${(wp.empFireRate * 100).toFixed(0)}% empLock=${(wp.empLockoutRate * 100).toFixed(0)}% ${clock} medHub=${medHub} medSec=${medSec} ${delta} (n_hub=${wp.hubSampleCount} n_sec=${wp.sectorSampleCount}) infilCap/g=${c.infiltratorCapturesPerGame.toFixed(2)}`,
    );
